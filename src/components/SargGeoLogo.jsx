 ...
 * Renders "SargGe" + custom interactive 3D Globe vector for the "O"
 */
export default function SargGeoLogo({ size = 'medium', className = '' }) {
  const isCompact = size === 'small';
  
  return (
    <div className={`sarggeo-logo-container ${size} ${className}`}>
      <span className="sarggeo-brand-text">
        SargGe
      </span>
      <div className="sarggeo-globe-wrapper" title="SargGeo Spatial Globe">
        ...
        >
          ...
          {/* Simplified Continent Vector Shapes */}
          <g fill="#38bdf8" fillOpacity="0.25">
            {/* North America */}
          ...
          {/* Longitudinal Arcs */}
          <g stroke="url(#grid-line-grad)" strokeWidth="1.2" fill="none">
            {/* Prime Meridian & Curving Meridians */}
          ...
          {/* Dynamic Orbital Axis Ring with Pulsing Satellite Node */}
          <g transform="rotate(-22 50 50)" className="sarggeo-globe-orbit">
            <ellipse
              cx="50"
              cy="50"
              rx="46"
